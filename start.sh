...
<<DESCRIPTION Setup and start the experiment from the physical host.
DESCRIPTION
...
FOLDERNAME="tmp/"
COPYFILES=("createPlot.py" "dependencies.sh" "containers.sh" "host.conf" "leecher.conf" "leecher.py" "lxcControl.py" "main.py" "main.sh" "seeder.conf" "seeder.py")
mkdir $FOLDERNAME
...
lxc-create --quiet --template download -n $CONTAINER --config $FOLDERNAME/host.conf -- -d ubuntu -r xenial -a amd64
